...
class background:
    ...
    def welcome(self,m,screen,size):
        ...
        pygame.draw.rect(screen,(0,0,0),rect=[0,size[1]-3,size[0],size[0]/13+3])
        self.draw_text(screen,f"CONNECT 4",self.font,(255,255,255),size[0]/2,size[1]+3*size[0]/65)
    ...
    def update_display(self,screen,cell,length,Turn_id):
        ...
        if Turn_id==1:
            pygame.draw.aacircle(screen,(53,6,62),cell,length-2*self.size[0]/65)

        else:
            pygame.draw.aacircle(screen,(0,0,139),cell,length-2*self.size[0]/65)
    ...
    def switch_player(self,screen,size,Turn,player1,player2):
        ...
        if Turn==player1:
            self.draw_text(screen,f"{player1}(Purple)'s turn",self.font,(255,255,255),size[0]/2,size[1]+3*size[0]/65)
        
        else:
            self.draw_text(screen,f"{player2}(Blue)'s turn",self.font,(255,255,255),size[0]/2,size[1]+3*size[0]/65)